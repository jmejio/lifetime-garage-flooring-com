...
- Two pages only: `index.html` (home) and `about.html`. Pages are generated from local Jinja2 templates, not hand-duplicated. Source of truth lives under `build/`:
  ...
  - `build/templates/_jsonld.jinja` — the `HomeAndConstructionBusiness` JSON-LD block, included on every page. Its `image`/`url` fields are templated from `SITE_BASE_URL`; only `sameAs` (the LinkedIn company URL, unrelated to the site's own domain) stays hardcoded.
